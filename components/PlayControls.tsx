...
export default function PlayControls({ showModal, onClose }: PlayControlsProps) {
  ...
  const quickMatch = () => {
    ...
    sock.once('match_found', ({ roomId }: { roomId: string }) => {
      onClose(); // Close modal when match is found
  ...
  const playWithFriend = () => {
    ...
    sock.once('player_joined', () => {
      onClose(); // Close modal when player joins
  ...
  return (
    <Modal title="Select Your Game Preferences" onClose={onClose} show={showModal}>
